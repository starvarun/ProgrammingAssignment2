...
makeCacheMatrix <- function(x = matrix()) {
  ...
  makeCacheMatrix <- function(x = matrix()) { ## argument with default value of "matrix"
    inv <- NULL                             ## initialize inv as NULL;
    ##will hold value of matrix inverse 
    set <- function(y) {                    ## define the set function to assign new 
      x <<- y                           ## value of matrix in parent environment
      inv <<- NULL                      ## if there is a new matrix,
      ##reset inv to NULL
    }
    ...
    ##where called
    list(set = set, get = get, setinverse = setinverse, getinverse = getinverse)  ## List created 
    ## $ operator
  }
}
